...
studyAreaGenerator <- function(url = NULL, destPath = NULL, Crs = NULL, 
  ...
  studyArea <- reproducible::Cache(reproducible::prepInputs, url = url,
                                   archive = archive, 
...
rtmGenerator <- function(url = NULL, destPath = NULL, 
  ...
  if (is.null(destPath))
    destPath <- tempdir()
  RTM <- reproducible::Cache(reproducible::prepInputs, url = url,
                             destinationPath = destPath,
                             studyArea = sA, 
                             useSAcrs = useSAcrs,
                             fun = "terra::rast",
...
makeListSACaribou <- function(studyArea, destPath){
  ...
  sa1 <- reproducible::Cache(reproducible::prepInputs, url = Boreal_Caribou_Revised_Study_Areas_2020,
                             targetFile = "Boreal_Caribou_Revised_Study_Areas_2020.shp",
                             studyArea = studyArea,
                             destinationPath = destPath,
                             fun = "terra::vect")
  sa2 <- reproducible::Cache(reproducible::prepInputs, url = BIO_ENR_WFE_BorealCaribou_RangePlanRegions_2020,
                             targetFile = "BIO_ENR_WFE_BorealCaribou_RangePlanRegions_2020.shp",
                             studyArea = studyArea,
                             destinationPath = destPath,
                             fun = "terra::vect")
  sa3 <- reproducible::Cache(reproducible::prepInputs, url = BIO_ENR_WFE_BorealCaribou_GNWT_NT1_range_2020,
...
copyExistingDisturbances <- function(from, to){
  if (file.exists(from)){
    reproducible::copyFile(from, to)
    message(paste0("File ", basename(from), " sucessfully copied from ", dirname(from), " to ", dirname(to)))
  } else {
    warning(paste0("File ", basename(from), " doesn't exist in ", dirname(from), ". Nothing copied."))
  }
    return(to)
}
